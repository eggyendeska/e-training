...
<div class="panel-body">
						<form class="form-horizontal" method="POST" action="{{ route('content.store') }}/{{ $content->id }}">
                        {{ csrf_field() }} {{ method_field('PUT') }} 
                                
								<div class="form-group" id="title_form">
                                    <label for="userTitle">Title</label>
                                    <input type="text" name="title" parsley-trigger="change" required
                                           placeholder="Enter content's title" class="form-control" id="title" value="{{ $content->title }}">
								@if ($errors->has('title'))
                                    <script>
								...
								<div class="form-group" id="source_form">
                                    ...
                                    <select class="form-control select2" name="sources_id" id="sources_id" required>
										...
										<option value="{{ $source->id }}" @if($content->sources_id == $source->id) Selected @endif>{{ $source->name }}</option>
										@endforeach
									</select>
                                </div>
								<div class="form-group" id="id_content_form">
									...
									<div class="input-group m-t-10">
										<input type="text" name="id_content" parsley-trigger="change"
											   placeholder="Enter content's id" class="form-control" id="id_content_code" value="{{ $content->id_content }}">
								...
								<div class="form-group" id="note_form">
                                    <label for="userName">Note (Private)</label>
                                         <textarea class="form-control" rows="5" name="note" id="note" >{{ $content->note }}</textarea>
                                </div>
								<div class="form-group" id="source_form">
                                    ...
                                    <select class="form-control select2" name="categories_id" id="categories_id" required>
										...
										<option value="{{ $cat->id }}" @if( $content->categories_id  == $cat->id) Selected @endif>{{ $cat->name }}</option>
										@endforeach     
									</select>
                                </div>
								<div class="form-group" id="tags_form">
                                    <input type="text" name="tags" parsley-trigger="change"
                                           placeholder="Enter content's Tags" class="form-control" id="tags" data-role="tagsinput" value="{{ $content->tags  }}">
								</div>
								<div class="form-group" id="source_form">
                                    <label for="role">Status</label>
									<p />
                                    <div class="radio radio-info radio-inline">
                                        <input type="radio" id="status1" value="1" name="status" @if($content->status==1)checked @endif>
                                        <label for="inlineRadio1"> Public </label>
                                    </div>
                                    <div class="radio radio-inline">
                                        <input type="radio" id="status2" value="0" name="status" @if($content->status==0)checked @endif>
