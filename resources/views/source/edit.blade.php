...
<div class="panel-body">
						<form class="form-horizontal" method="POST" action="{{ route('source.store') }}/{{ $source->id }}">
                        {{ csrf_field() }} {{ method_field('PUT') }} 
                                
								<div class="form-group" id="name_form">
                                    <label for="userName">Name</label>
                                    <input type="text" name="name" parsley-trigger="change" required
                                           placeholder="Enter source's name" class="form-control" id="name" value="{{ $source->name }}">
								@if ($errors->has('name'))
                                    <script>
								...
								<div class="form-group" id="url_form">
                                    ...
                                    <input type="text" name="url" parsley-trigger="change"
                                           placeholder="Enter source's URL" class="form-control" id="url" value="{{ $source->url }}">
								...
								<div class="form-group" id="embed_form">
                                    ...
                                    <input type="text" name="embed_code" parsley-trigger="change"
                                           placeholder="Enter source's embed code" class="form-control" id="embed_code" value="{{ $source->embed_code }}">
								...
								<div class="form-group" id="example_form">
                                    ...
                                    <textarea class="form-control" rows="5" name="example" id="description" required>{{ $source->example }}</textarea>
